...
public class JoglRenderDevice implements RenderDevice {
    ...
    public RenderImage createImage(final String filename, final boolean filterLinear) {
        return new JoglRenderImage(filename, filterLinear);
    }
    ...
    public RenderFont createFont(final String filename) {
        return new JoglRenderFont(filename, this);
    }
    ...
    public void renderFont(final RenderFont font, final String text, final int x, final int y,
            final Color color, final float fontSize) {
        log.fine("renderFont()");
        final GL gl = GLContext.getCurrentGL();
        if (!currentTexturing) {
            gl.glEnable(GL.GL_TEXTURE_2D);
        ...
        setBlendMode(BlendMode.BLEND);
        if (color == null) {
            ((JoglRenderFont) font).getFont().drawStringWithSize(x, y, text, fontSize);
        }
        else {
            ((JoglRenderFont) font).getFont().renderWithSizeAndColor(x, y, text, fontSize,
