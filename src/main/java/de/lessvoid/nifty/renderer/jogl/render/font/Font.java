...
import de.lessvoid.nifty.renderer.jogl.render.JoglRenderFont;
import de.lessvoid.nifty.renderer.jogl.render.JoglRenderImage;
import de.lessvoid.nifty.renderer.jogl.render.font.ColorValueParser.Result;
...
public class Font {
    ...
    public final boolean init(final String filename) {
        ...
        for (int i = 0; i < font.getTextures().length; i++) {
            textures[i] = new JoglRenderImage(extractPath(filename) + font.getTextures()[i], true);
        }
   ...
   */
    ...
    public void renderWithSizeAndColor(int x, int y, String text, float size, float r, float g,
            float b, float a) {
        final GL2 gl = GLContext.getCurrentGL().getGL2();
        // enableBlend();
    ...
    private void internalRenderText(final int xPos, final int yPos, final String text,
            final float size, final boolean useAlphaTexture, final float alpha) {
        final GL2 gl = GLContext.getCurrentGL().getGL2();
        ...
        for (int i = 0; i < text.length(); i++) {
            ...
            if (charInfoC != null) {
                ...
                kerning = JoglRenderFont.getKerning(charInfoC, nextc);
                characterWidth = (charInfoC.getXadvance() * size + kerning);

                gl.glLoadIdentity();
                gl.glTranslatef(x, yPos, 0.0f);

                gl.glTranslatef(0.0f, getHeight() / 2, 0.0f);
                ...
                if (isSelection()) {
                    if (i >= selectionStart && i < selectionEnd) {
                        ...
                        gl.glVertex2i((int) characterWidth, 0 + getHeight());
                        gl.glVertex2i(0, 0 + getHeight());
