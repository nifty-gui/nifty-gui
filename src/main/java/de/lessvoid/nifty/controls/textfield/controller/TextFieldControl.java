...
public class TextFieldControl extends AbstractController {
    ...
    public void onStartScreen() {
        ...
        this.lastVisibleCharacterIndex = FontHelper.getVisibleCharactersFromStart(
                textRenderer.getFont(), this.textField.getText(), fieldWidth, 1.0f);

        updateCursor();
    }

    public void onClick(final int mouseX, final int mouseY) {
        String visibleString = textField.getText().substring(firstVisibleCharacterIndex,
    ...
    private void updateCursor() {
        ...
        cursorElement.setConstraintY(new SizeValue(
                (element.getHeight() - cursorElement.getHeight()) / 2 + CURSOR_Y + "px"));
    ...
    private void checkBounds(final String text, final TextRenderer textRenderer) {
        ...
        if (firstVisibleCharacterIndex > textLen) {
            ...
            firstVisibleCharacterIndex = FontHelper.getVisibleCharactersFromEnd(
                    textRenderer.getFont(), text, fieldWidth, 1.0f);
        }
    }
    ...
    private void calcLastVisibleIndex(final TextRenderer textRenderer) {
        ...
        if (firstVisibleCharacterIndex < currentText.length()) {
            ...
            int lengthFitting = FontHelper.getVisibleCharactersFromStart(textRenderer.getFont(),
                    textToCheck, fieldWidth, 1.0f);
