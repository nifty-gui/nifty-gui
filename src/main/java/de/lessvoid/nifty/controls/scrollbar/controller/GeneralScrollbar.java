...
public class GeneralScrollbar extends AbstractController {
  ...
  private void calcPageSize() {
    ...
    scrollBar.resizeHandle(
        scrollPos,
        new SizeValue((int) (scrollBar.translateValue(element.getWidth(), element.getHeight()) / pages) + "px"));
  ...
  private void changeSliderPosFromMouse(final int mouseX, final int mouseY) {
    int newPos = scrollBar.translateValue(mouseX, mouseY) - scrollBar.translateValue(element.getX(), element.getY())
        - startMouse;
   ...
   * 
   * @param worldValue
   *          the current value of the scroll bar must be in range [minValue,
   *          maxValue]
   */
