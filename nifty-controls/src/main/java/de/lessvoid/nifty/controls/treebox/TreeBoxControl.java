...
public class TreeBoxControl extends AbstractController implements TreeBox {
    ...
    private void setListBox(final String name) {
        if (treeListBox == null) {
            treeListBox = (ListBox<TreeEntryModelClass>) element.findNiftyControl(name, ListBox.class);
        }
    }

    private int addTreeItem(ListBox<TreeEntryModelClass> treeListBox, TreeItem treeItem, int currentIndent, int itemIndex) {
        if (currentIndent != 0) {
            treeListBox.insertItem(new TreeEntryModelClass(indentWidth * currentIndent, treeItem), itemIndex);
            itemIndex++;
        }
        if (treeItem.isExpanded()) {
            for (Object childItem : treeItem.getTreeItems()) {
                itemIndex = addTreeItem(treeListBox, (TreeItem) childItem, currentIndent + 1, itemIndex);
            }
        }
        return itemIndex;
    }
